...
language: ko-KR
author: 김민조
title: "숫자를 모두에게 보여주면 판단도 분산될까?"
subtitle: "셀프서비스 대시보드의 자율성은 누가 정한 정보 경계 위에 놓여 있다"
description: "대시보드가 정보 접근을 넓힌 뒤에도 판단이 분산되지 않는 이유를 정보 경계, 관계 변화, 엇갈린 성과 신호의 관점에서 살펴본다."
enableToc: true
source_draft: "04_Drafts/2026-08-02_draft_dashboard-information-boundary.md"
source_ready: "2026-08-02_ready_dashboard-information-boundary"
human_note: "개념 다양화가 멈춘 상태를 바로잡은 뒤, 확장된 개념 원자를 반영해 새 글을 발행"
human_note_status: incorporated
target_format: public_research_essay
topic_balance_axis: management_control_information_boundary_relational_dynamics
...
reference_truth_audit_date: 2026-08-02
self_plagiarism_review:
  status: pass
  exact_sentence_overlap_published_20: 0
  semantic_distance_from_recent_articles: high
style_profile:
  opening_type: object_metaphor
  rhythm_type: continuous_essay_single_motif
  theory_placement: interwoven_after_numeric_example
  ending_type: motif_return_quiet_aftertaste
sharpness_gate:
  status: pass
  lasting_sentence: "판단은 사람들이 화면을 볼 수 있을 때가 아니라, 화면이 놓친 것을 말하고 프레임을 고칠 수 있을 때 비로소 넓어진다."
hook_gate:
  status: hook_pass
readability_gate:
  status: pass
  automation_check: pass
  body_characters: 2454
  max_paragraph_characters: 209
...
셀프서비스 대시보드는 이 프레임을 알아차리기 어렵게 만들기도 한다. 사용자는 기간을 바꾸고 제품과 고객을 나누며 자유롭게 분석한다. 직접 조작할 수 있으니 판단권도 자신에게 온 것처럼 느낀다.
...
다음 월간회의에서 대시보드를 뜯어고칠 필요는 없다. 97% 옆에 `아직 설명되지 않은 신호`라는 빈칸 하나를 두는 것으로 시작할 수 있다. 긴급 출고, 반복 재작업, 예외 승인처럼 공식 지표와 어긋나는 현상을 그곳에 잠시 올려놓는다.

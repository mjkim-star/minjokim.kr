...
AI가 문장을 더 잘 만들수록 오히려 회사는 더 분명히 말해야 한다. 무엇을 원하는지보다, 무엇을 좋은 결과로 볼 것인지. 어디까지 맡길 것인지보다, 어디서 사람이 판단할 것인지. 결과물을 만들 것인지보다, 그 결과물이 어떤 결정과 행동에 쓰일 것인지.

이 글의 질문은 이것이다.

회사에서 AI 프롬프트는 더 이상 중요한가? 중요하다면, 무엇이 중요해졌는가?
...
이렇게 쓰면 프롬프트는 AI에게 던지는 문장을 넘어선다. 그 프롬프트는 회사가 고객을 어떻게 대하고, 어떤 책임을 인정하며, 어디서 사람의 판단을 남겨둘 것인지 설명하는 문장이 된다.

이 점에서 반복 업무의 프롬프트는 직무기술서의 일부 기능을 드러낸다. 일을 설명하고, 필요한 결과와 기준을 밝히고, 사람과 AI의 역할을 나누게 만들기 때문이다.
...
프롬프트는 조직이 일을 어떻게 이해하고 있는지 드러낸다. 왜 필요한지 설명하지 못하면 프롬프트도 흐릿하다. 판단 기준이 없으면 AI 결과도 흔들린다. 책임 경계가 없으면 AI가 만든 산출물은 빠르게 나오지만, 조직의 일하는 방식은 바뀌지 않는다.

그래서 AI 프롬프트는 직무기술서가 아니다. 다만 회사 안에서 반복되는 업무에서는 직무기술서의 일부 기능을 드러낼 수 있다.

문서를 하나 더 만들자는 뜻도 아니다. AI에게 일을 시키는 문장을 통해 그 결과가 왜 필요한지, 누가 판단해야 하는지, 무엇이 반복되어야 하는지를 다시 보자는 뜻이다.

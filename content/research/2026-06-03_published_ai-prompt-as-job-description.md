...
subtitle: "프롬프트는 명령문이 아니라, 일의 의도와 판단 기준을 설명하게 만드는 문장이다"
language: ko-KR
author: 김민조
...
human_note: "AI에게 일을 시킬 때 어려운 것은 프롬프트 문장만이 아니라, 그 일의 의도와 판단 기준을 조직 안에서 충분히 설명하지 못하는 경우다."
...
무엇을 해야 하는가. 왜 해야 하는가. 이 결과가 어디에 쓰이는가. 어떤 자료를 봐야 하는가. 무엇을 좋은 결과로 볼 것인가. 어디까지는 AI가 처리하고, 어디부터는 사람이 판단해야 하는가.

이 질문에 답하지 못하면 프롬프트는 길어져도 좋아지지 않는다.

그래서 이 글의 질문은 이것이다.

회사에서 AI 프롬프트는 단순한 명령어인가, 아니면 조직이 일의 의도와 기준을 다시 설명하게 만드는 계기인가?
...
이렇게 쓰면 프롬프트는 AI에게 던지는 문장을 넘어선다. 그 프롬프트는 회의록이라는 일의 의도와 판단 기준을 설명하는 문장이 된다.

---
title: 김민조 연구노트
description: 조직, 전략, 동적역량, AI 전환을 이론과 방법론의 관점에서 탐구하는 공개 연구노트
date: 2026-05-27
published: 2026-05-27
enableToc: false
---

# 김민조 연구노트

<section class="home-section latest-section">
  <p class="section-label">Latest</p>
  <h2>최근 글</h2>
  <!-- 최근 글은 원문 기준 최대 3개 묶음만 노출합니다. 번역본은 원문 바로 아래에 둡니다. -->
  <div class="latest-list">
    <div class="latest-group">
      <a class="latest-original" href="./research/2026-05-29_published_sme-ambidexterity-boundary">
        <span>"양손잡이 조직"은 언제 미덕이 아니라 비용이 되는가?</span>
        <time>2026.05.29</time>
      </a>
      <a class="latest-translation" href="./research/2026-05-29_published_zh_sme-ambidexterity-boundary">
        <span><small>중문 번역</small> 双元组织何时不再是优点，而成为成本？</span>
        <time>2026.05.29</time>
      </a>
    </div>
    <div class="latest-group">
      <a class="latest-original" href="./research/2026-05-27_published_ai-adoption-dynamic-capability-mediation">
        <span>AI를 도입했는데 왜 회사는 달라지지 않는가?</span>
        <time>2026.05.27</time>
      </a>
      <a class="latest-translation" href="./research/2026-05-28_published_en_ai-adoption-dynamic-capability-mediation">
        <span><small>영문 번역</small> Why Do Companies Remain the Same After Adopting AI?</span>
        <time>2026.05.28</time>
      </a>
    </div>
  </div>
</section>

<hr class="home-divider" />

<section class="home-section intro-section">
  <div class="intro-mark-row">
    <img class="intro-mark" src="/static/icon.png" alt="" />
    <p class="section-label">About</p>
  </div>

<p class="intro-line">경영학 박사이자 경영지도사입니다.</p>

<p class="intro-line">전략, 동적역량, 의사결정, AI 전환을 연구합니다.</p>

<p class="intro-line">중심 질문은 하나입니다.</p>

<p class="intro-question">좋은 이론은 왜 현장에서 빈번하게 멈추는가.</p>

<p class="intro-body">이 질문에서 글은 시작합니다. 기존 이론이 설명하지 못한 빈칸을 찾고, 그 위에 새로운 방법론을 모색하고 개념 모델을 확장하는 것이 이곳의 작업입니다.</p>
</section>

<hr class="home-divider" />

<section class="home-section scope-section">
  <p class="section-label">Research Scope</p>

<div class="research-cards">
  <section>
    <strong>이론의 경계조건</strong>
    <span>좋은 이론이 언제 현장에서 멈추는지 살핍니다.</span>
  </section>
  <section>
    <strong>방법론과 진단틀</strong>
    <span>방법론을 모색하고, 개념 모델을 확장합니다.</span>
  </section>
  <section>
    <strong>AI와 업무 구조</strong>
    <span>AI를 기술이 아니라 일과 판단의 재설계로 읽습니다.</span>
  </section>
  <section>
    <strong>조직의 실행 회로</strong>
    <span>판단, 책임, 회의, 학습이 실행으로 이어지는 조건을 봅니다.</span>
  </section>
</div>
</section>

<hr class="home-divider" />

<section class="home-section publications-section">
  <p class="section-label">Publications</p>
  <h2>논문과 저서</h2>
  <p class="publication-intro">연구 범위와 연결되는 학위논문과 공저 이력을 공개 가능한 범위에서 정리합니다.</p>
  <div class="publication-grid publication-preview-grid">
    <a class="publication-card" href="./research/papers-and-books/papers/master-thesis-sungkyunkwan-2015">
      <span class="publication-type">석사논문</span>
      <strong>한국제조기업의 재료비 원가행태에 관한 연구</strong>
      <small>성균관대학교 · 2015년 · 원가행태와 관리회계</small>
    </a>
    <a class="publication-card" href="./research/papers-and-books/papers/phd-higher-order-resources-performance-persistence">
      <span class="publication-type">박사논문</span>
      <strong>고차자원이 성과 지속성에 미치는 영향</strong>
      <small>한양대학교 · 2025년 · 고차자원과 성과 지속성</small>
    </a>
    <a class="publication-card" href="./research/papers-and-books/books/barun-succession-guide">
      <span class="publication-type">저서</span>
      <strong>바른 승계 길라잡이</strong>
      <small>공저 · 승계 장면에서 본 의사결정과 조직 전환</small>
    </a>
  </div>
  <a class="publication-index-link" href="./research/papers-and-books/">아카이브 전체 보기</a>
</section>
